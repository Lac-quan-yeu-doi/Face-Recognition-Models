...
from utils.criterion import SphereFaceNet, CosFaceNet, ArcFaceNet
from utils.dataset import LFWDataset, CASIAwebfaceDataset, LFWPairDataset
...
def main():
    ...
    print(f"{'*'*10} ARCFACE MODEL {'*'*10}")

    model_folder_path = f'{WORKING_PATH}/models'
    os.makedirs(model_folder_path, exist_ok=True)
    model_final_path = f'{model_folder_path}/arcface_final.pth'
    model_best_path = f'{model_folder_path}/arcface_best.pth'
    ...
    model = ArcFaceNet(num_classes=train_dataset.num_of_identities).to(device)
    ...
    test_pairs_dataset = LFWPairDataset(root_dir=f'{DATASET_PATH}/Labeled Faces in the Wild (LFW)', 
                                       ...
                                       transform=transform)
    combined_pairs_dataset = torch.utils.data.ConcatDataset([train_pairs_dataset, test_pairs_dataset])
